...
def main(args):
    ...
    if args.dataset == 'CelebA':
        metadata_path = "/data/celebA/data/metadata.csv"
    ...
    my_csv=merged_csv[['image_id','Blond_Hair','Male','y_pred_None_epoch_30_val','our_spurious','spurious','wrong_1_times']]

    print(my_csv.loc[(my_csv['wrong_1_times']==1) & (my_csv['our_spurious']==False) & (my_csv['spurious']==False)& (my_csv['Male']==1)])
    with open('spurious.txt','a') as f:
        f.write('----------'+str(args.final_epoch)+'----------\n')
        f.write("Number of our spurious: "+str(np.sum(merged_csv["our_spurious"]))+"\n")
        f.write("Number of our nonspurious:"+str(np.sum(merged_csv["our_nonspurious"]))+"\n")
    ...
    with open('spurious.txt','a') as f:
        f.write("Spurious precision"+str(spur_precision)+"\n")
        f.write("Spurious recall"+str(spur_recall)+"\n")
        f.write('---------------------'+"\n")
    exit()
