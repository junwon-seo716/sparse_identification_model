...
def train(
    ...
    pruning_config = IterativePruningConfig(
        begin_pruning_step=12000 ,
        end_pruning_step=-1,
        policy_begin_step=12000,
        policy_end_step=36000,
        pruning_fn="unstructured_magnitude",
        pruning_fn_default_kwargs={"target_sparsity": 0}
    )
    print(pruning_config)

    pruning_scheduler = IterativePruningScheduler(model, pruning_config)
    try:
        model.load_state_dict(torch.load('./sparse_42/ERM_sparse/log_erm_spar_0995_no_reweight_LR_seed_42/60_model.pth'))
    except:
